...
let toolVersion() =
    let projectFilePath = Path.Combine(serverPath, "Server.fsproj")
...
let createAndPublishArtifacts() =
    let version = toolVersion()
    let github = GitHubClient(ProductHeaderValue "PulumiToolPlugin")
...
Target.create "LocalNugetBundle" (fun _ ->
    ...
    let nugetPackage = Directory.EnumerateFiles(outputPath, "Server.*.nupkg", SearchOption.AllDirectories).First()
    ...
    let serverDll = Directory.EnumerateFiles(outputPath, "Server.dll", SearchOption.AllDirectories).First()
